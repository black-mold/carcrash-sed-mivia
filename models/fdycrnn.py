...
class Dynamic_conv2d(nn.Module):
    ...
    def forward(self, x): #x size : [bs, in_chan, frames, freqs]
        ...
        if self.bias is not None:
            ...
            output = F.conv2d(x, weight=aggregate_weight, bias=aggregate_bias, stride=self.stride, padding="same")
        else:
            output = F.conv2d(x, weight=aggregate_weight, bias=None, stride=self.stride, padding="same")
...
class attention2d(nn.Module):
    def __init__(self, in_planes, kernel_size, stride, padding, n_basis_kernels, temperature, pool_dim):
        ...
        if not pool_dim == 'both':
            self.conv1d1 = nn.Conv1d(in_planes, hidden_planes, kernel_size, stride=stride, padding="same", bias=False)
            self.bn = nn.BatchNorm1d(hidden_planes)
            self.relu = nn.ReLU(inplace=True)
            self.conv1d2 = nn.Conv1d(hidden_planes, n_basis_kernels, 1, bias=True, padding="same")
    ...
    def __init__(self,
        ...
        def conv(i, normalization="batch", dropout=None, activ='relu'):
            ...
            if DY_layers[i] == 1:
                ...
                cnn.add_module("conv{0}".format(i), nn.Conv2d(in_dim, out_dim, kernel[i], stride[i], padding="same"))
