...
from engine_kd import FeatureExtractor
...
def train():
    ...
    model_student = importlib.import_module('models.' + config['model_student']).__getattribute__("MainModel")
    model_student =  model_student(**config['model_student_config'])

    model_teacher = importlib.import_module('models.' + config['model_teacher']).__getattribute__("MainModel")
    model_teacher =  model_teacher(**config['model_teacher_config'])

    optimizer = importlib.import_module("optimizer." + config['optimizer']).__getattribute__("Optimizer")
    optimizer = optimizer(model_student.parameters(), **config['optimizer_config'])

    loss_function_bce = importlib.import_module("loss." + config['loss_bce']).__getattribute__("loss_function")
    loss_function_mse = importlib.import_module("loss." + config['loss_mse']).__getattribute__("loss_function")
    ...
    # load pre-trained weight for teacher model



    # ⚡⚡  3. Set 'engine' for training/validation and 'Trainer' 
    engine = FeatureExtractor(preprocess = preprocess, 
                              model_student = model_student, 
                              model_teacher = model_teacher,
                              loss_function_bce = loss_function_bce,
                              loss_function_mse = loss_function_mse,
...
def test():
    ...
    # ⚡⚡  3. Load model
    feature_extractor = FeatureExtractor.load_from_checkpoint(
        model_teacher = model_teacher,
        model_student = model_student,
